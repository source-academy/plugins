...
const generalNode = (n: number, ...children: SerializedDataVisualizerNode[]): SerializedDataVisualizerNode => {
  if (children.length === 0) {
    return pair(leaf(n), empty());
  }
  const childrenList = children.reduceRight<SerializedDataVisualizerNode>((acc, c) => pair(c, acc), empty());
