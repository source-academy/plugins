...
export class BinaryTreeDrawer extends OriginalDrawer {
  ...
  draw(x: number, y: number, key: number): React.ReactElement {
    ...
    if (!this.classification.isBinaryTree) {
      return (
  ...
  protected drawNode(
    ...
    if (node instanceof FunctionTreeNode) {
      ...
      const treeDepth = this.classification.layout?.treeDepth ?? 0;
      node.children?.forEach((childNode, index) => {
        let myY;
        let myX;
        let scalerV = Math.round(
          Math.pow(2, treeDepth) / Math.pow(2, Math.round(y / (6 * Config.BoxHeight))),
        );
        scalerV--;
