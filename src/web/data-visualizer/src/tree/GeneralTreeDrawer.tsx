 ...
 */
export class GeneralTreeDrawer extends ClassicDrawer {
  constructor(
    tree: Tree,
  ...
  draw(x: number, y: number, key: number): React.ReactElement {
    ...
    if (!this.classification.isGeneralTree) {
      return (
    ...
    const longestNodePos = this.classification.layout?.longestNodePos ?? 0;
    return (
      <Stage
        key={key}
  ...
  protected drawNode(
    ...
    if (node instanceof FunctionTreeNode) {
      ...
      const layout = this.classification.layout;
      const longest = layout?.nodeCountByDepth[0] ?? 0; // e.g. 3
      this.runningX2 = (Config.NWidth + Config.BoxWidth) * (longest + 1);
      this.downCOUNTER = layout?.treeDepth ?? 0;
