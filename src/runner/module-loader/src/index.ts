import { CHANNEL_ID, ModuleLoaderMessageType, RUNNER_ID, type ModuleLoaderMessage } from "@sourceacademy/common-module-loader";
import {
  checkIsPluginClass,
  type IChannel,
  type IConduit,
    ...
    return new Promise((resolve, reject) => {
      const handleResponse = async (msg: ModuleLoaderMessage) => {
        ...
        if (msg.type === ModuleLoaderMessageType.MODULE_RESPONSE) {
          const plugin = await import(msg.moduleURL)
            .then(module => {
              return module.default;
            })
          const pluginObj = this.__conductor.registerPlugin(plugin, {
            tabs: msg.tabs,
